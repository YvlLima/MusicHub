...
// Criar Playlist
app.post("/api/playlists", autenticarToken, async (req, res) => {
  const { nome, descricao, capa, privada } = req.body;
  const criado_por = req.user.username;

  if (!nome || !nome.trim()) {
    return res.status(400).json({ erro: "Nome da playlist é obrigatório." });
  }

  const fotoCapa = capa && validarImagemBase64(capa) ? capa : "imagens/pfp.png";
  const ehPrivada = privada ? 1 : 0;

  try {
    const result = await pool.query(
      `INSERT INTO playlists (nome, descricao, capa, criado_por, privada)
       VALUES ($1, $2, $3, $4, $5) RETURNING *`,
      [nome.trim(), descricao || "", fotoCapa, criado_por, ehPrivada],
    );

    await registarLog(criado_por, "CRIOU PLAYLIST", nome.trim());
    res.json({
...
// Listar Playlists (Públicas + Próprias)
app.get("/api/playlists", async (req, res) => {
  ...
  if (token) {
    try {
      ...
      // Token inválido, prosseguir como anónimo
    }
  }

  try {
    const query = username
      ? `SELECT p.*, (SELECT COUNT(*)::int FROM playlist_faixas pf WHERE pf.playlist_id = p.id) as total_faixas
         FROM playlists p
         WHERE p.privada = 0 OR p.criado_por = $1
         ORDER BY p.id DESC`
      : `SELECT p.*, (SELECT COUNT(*)::int FROM playlist_faixas pf WHERE pf.playlist_id = p.id) as total_faixas
         FROM playlists p
         WHERE p.privada = 0
    ...
    res.status(500).json({ erro: "Erro ao obter playlists." });
  }
});

// Obter Detalhes e Faixas de uma Playlist
app.get("/api/playlists/:id", async (req, res) => {
  const { id } = req.params;

  try {
    const pRes = await pool.query("SELECT * FROM playlists WHERE id = $1", [
      id,
    ]);
    ...
    const faixasRes = await pool.query(
      `SELECT pf.id as faixaid, pf.item_id, pf.ordem, pf.adicionado_em
       FROM playlist_faixas pf
       WHERE pf.playlist_id = $1
       ORDER BY pf.ordem ASC, pf.id ASC`,
      [id],
    );

    res.json({ playlist, faixas: faixasRes.rows });
  } catch (err) {
    console.error("Erro ao obter playlist:", err);
    res.status(500).json({ erro: "Erro ao carregar detalhes da playlist." });
  }
});

// Adicionar Faixa a uma Playlist
app.post("/api/playlists/:id/tracks", autenticarToken, async (req, res) => {
  const { id } = req.params;
  const { item_id } = req.body;
  const username = req.user.username;

  if (!item_id) {
    return res.status(400).json({ erro: "Item ID é obrigatório." });
  }

  try {
    const pRes = await pool.query(
      "SELECT criado_por FROM playlists WHERE id = $1",
      [id],
    );
    if (pRes.rows.length === 0) {
      return res.status(404).json({ erro: "Playlist não encontrada." });
    ...
    if (pRes.rows[0].criado_por !== username && req.user.is_admin !== 1) {
      return res
        .status(403)
        .json({ erro: "Apenas o criador pode editar esta playlist." });
    }

    await pool.query(
      `INSERT INTO playlist_faixas (playlist_id, item_id)
       VALUES ($1, $2)
       ON CONFLICT (playlist_id, item_id) DO NOTHING`,
      [id, item_id],
    );

    res.json({ mensagem: "Faixa adicionada à playlist!" });
  } catch (err) {
    console.error("Erro ao adicionar faixa à playlist:", err);
    res.status(500).json({ erro: "Erro ao adicionar faixa." });
  }
});

// Remover Faixa de uma Playlist
app.delete(
  "/api/playlists/:id/tracks/:itemId",
  autenticarToken,
  async (req, res) => {
    ...
    try {
      ...
      if (pRes.rows[0].criado_por !== username && req.user.is_admin !== 1) {
        return res
          .status(403)
          .json({ erro: "Sem permissão para alterar esta playlist." });
      }

      await pool.query(
        "DELETE FROM playlist_faixas WHERE playlist_id = $1 AND item_id = $2",
        [id, itemId],
      );

      res.json({ mensagem: "Faixa removida da playlist." });
    } catch (err) {
      console.error("Erro ao remover faixa:", err);
      res.status(500).json({ erro: "Erro ao remover faixa da playlist." });
    }
  },
);

// Eliminar Playlist
app.delete("/api/playlists/:id", autenticarToken, async (req, res) => {
  const { id } = req.params;
  const username = req.user.username;

  try {
    const pRes = await pool.query(
      "SELECT criado_por, nome FROM playlists WHERE id = $1",
      [id],
    );
    if (pRes.rows.length === 0) {
      return res.status(404).json({ erro: "Playlist não encontrada." });
    ...
    if (pRes.rows[0].criado_por !== username && req.user.is_admin !== 1) {
      return res
        .status(403)
        .json({ erro: "Sem permissão para eliminar esta playlist." });
    }

    await pool.query("DELETE FROM playlists WHERE id = $1", [id]);
    await registarLog(username, "ELIMINOU PLAYLIST", pRes.rows[0].nome);

    res.json({ mensagem: "Playlist eliminada com sucesso!" });
  } catch (err) {
    console.error("Erro ao eliminar playlist:", err);
    res.status(500).json({ erro: "Erro ao eliminar playlist." });
  }
});
