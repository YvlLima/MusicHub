...
// ==========================================
// 4. PAINEL DE ADMINISTRAÇÃO & MÚSICA DE FUNDO
// ==========================================

var ytPlayerFundo = null;
var tipoMusicaFundoAtual = null;
...
const LIMITE_BYTES_MUSICA = 10 * 1024 * 1024;
...
async function carregarMusicaFundoSite() {
  const container = document.getElementById("container-musica-fundo");
...
function tocarMusicaAtual() {
  ...
  const spanNome = document.getElementById("nome-musica-atual");
  if (spanNome) {
    spanNome.innerText =
      musica.nome ||
      (musica.tipo === "youtube" ? "YouTube Stream" : "Áudio Local");
  }
...
function definirVolumeMusicaFundo(valor) {
  ...
  localStorage.setItem(CHAVE_VOLUME_MUSICA, vol);
  const audio = document.getElementById("audio-player-fundo");

  if (audio) audio.volume = vol;
  if (ytPlayerFundo && typeof ytPlayerFundo.setVolume === "function") {
    ytPlayerFundo.setVolume(vol * 100);
  }
}
...
function atualizarInterfacePlayer() {
  const btnPlayPause = document.getElementById("btn-play-pause-fundo");
  const sliderVol = document.getElementById("slider-volume-fundo");
  const iconeStatus = document.getElementById("icone-player-status");

  if (btnPlayPause) {
    btnPlayPause.innerText = estaSilenciado() ? "▶" : "⏸";
  }
  if (iconeStatus) {
    iconeStatus.innerText = estaSilenciado() ? "🔇" : "🔊";
  }
  if (sliderVol) {
    sliderVol.value = obterVolumeGuardado();
  }
}
...
window.addEventListener("DOMContentLoaded", async () => {
  ...
  if (typeof carregarMusicaFundoSite === "function") {
    carregarMusicaFundoSite();
  }
});
